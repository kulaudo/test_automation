...
def larger_than_two(I):
    return I > 2
    
def has_three_elements(I):
    return len(I) >= 3

def is_a_greeting(I):
    return I in ["Hello", "Goodbye"]
...
class FooTestCase(unittest.TestCase):
    ...
    @needs_yaml
    @file_data('test_data\\test_data_dict.yaml')
    def test_file_data_yaml_dict(self, value):
        self.assertTrue(has_three_elements(value))

    @needs_yaml
    @file_data('test_data\\test_data_list.yaml')
    def test_file_data_yaml_list(self, value):
        self.assertFalse(self.assertTrue(is_a_greeting(value)))
